...
def create_xlsx(input_data, filename, debug):
    ...
    try:
        ...
        if debug:
            print("THE FOLLOWING ERROR MAY BE EXPECTED AND ONLY SHOWS WHEN DEBUG=True: \n")
            raise
    except:
        raise
    finally:
...
def create_all_reports(extracted_data, config):
    ...
    try:
        ...
        if config["export_type"] in ["XLSX", "BOTH"]:
            if config.get("output_directory"):
                create_xlsx(extracted_data, f"{config['output_directory']}\\ad_report.xlsx", config["DEBUG"])
            else:
                create_xlsx(extracted_data, "ad_report.xlsx", config["DEBUG"])
    except Exception:
        raise
